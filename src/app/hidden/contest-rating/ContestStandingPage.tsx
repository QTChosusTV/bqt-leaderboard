...
export default function ContestStandingPage() {
  ...
  useEffect(() => {
    ...
    const fetchStanding = async () => {
      ...
      const oldEloDisplay = elo; //getDisplayedElo(elo, contestCount);
      const isRated = contest ? getDisplayedElo(oldEloDisplay, contestCount) >= (contest.elo_min ?? 0) && getDisplayedElo(oldEloDisplay, contestCount) <= (contest.elo_max ?? 4000) : true;
      ...
      const ratedParticipants = (standingData ?? [])
        ...
        // console.log("=== Rated Participants ===");
          ratedParticipants.forEach((p, idx) => {
            const disp = p.elo; /*getDisplayedElo(
              p.elo,
      ...
      const newEloDisplay = newElo; //getDisplayedElo(newElo, contestCount + 1);
      const delta = Math.round(newEloDisplay - oldEloDisplay);

      console.log(rank, " ", s.user_id, " ", s.score, " ", s.penalty);
      console.log(rank, " ", s.user_id, " ", s.score, " ", s.penalty);
          ...
          <table className="w-full border-collapse text-xs table-auto text-center align-middle bg-gray-800">
            <thead className="bg-gray-700 text-white">
              <tr className={styles.cspTable}>
                ...
                <th className="px-4 py-2 text-center border">DispElo</th>
                  {((now <= timeEnd) || true) && (
                    <>
              ...
              {standings.map((s, index) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-1 py-2 text-center border text-yellow-400">
                      <span style={{ fontFamily: "Oswald" }} className={getEloClass(getDisplayedElo(s.old_display_elo, s.contest_count))}>
                        {getDisplayedElo(s.old_display_elo, s.contest_count)}
                      </span>
                    </td>
                    {((now <= timeEnd) || true) && (
                      <>
